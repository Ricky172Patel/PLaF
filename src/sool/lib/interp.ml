...
let rec is_subclass : string -> string -> class_env -> exp_val ea_result =
  fun c_name1 c_name2 c_env ->
    (match List.assoc_opt c_name2 c_env with
    | None -> error ("is_subclass: class " ^ c_name2 ^ " not found")
    | Some _ -> 
      (match List.assoc_opt c_name1 c_env with 
      | None -> return (BoolVal false)
      | Some (super,_,_) -> if c_name1 = c_name2 then return (BoolVal true) else (is_subclass super c_name2 c_env)))
...
and   
  ...
  | IsInstanceOf(e,id) -> 
    eval_expr e >>= fun ev ->
    (match ev with
    | ObjectVal(c_name, _) -> (is_subclass c_name id !g_class_env)
    | _ -> error "IsInstanceOf: Expected ObjectVal")
  (* Debug *)
  | Debug(_e) ->
